...
class Ori_Data():
    ...
    def get_positive_marker_gene_parallel(self, log2fc=0.5, pval=0.1, gene_list_len=20,cores=2):
        pool = mp.Pool(cores)
    ...
    def _supercell(self):
        ...
        sc.pp.log1p(ad_all)
        ad_all = ad_all.to_df()
        self.adata.obs.insert(loc=3, column='new_leiden', value=0)
...
class SCRIPro_Multiome():
    ...
    def get_tf(self,target_h5):
        ...
        # 这里的row是一个Series，它的index是['column', 'value']
            tf_name = row['column']
        ...
        # pivot()函数会创建一个MultiIndex，你可以使用reset_index()函数将行索引转换回普通列
        chip_matrix2_new = chip_matrix2_new.reset_index()
        # 使用rename_axis()函数删除列索引名称
...
class SCRIPro_RNA(SCRIPro_Multiome):  # 定义派生类
    def __init__(self, cores, species, Ori_Data, assays):
        super().__init__(cores, species, Ori_Data)  # 使用super()调用基类的构造方法
        self.assays = assays
