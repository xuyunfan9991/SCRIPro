...
def process_and_merge(df, folder_path, n_num=20):
    # 数据集分割份数和最大进程数
    num_partitions = min(n_num, len(df))  # 分割份数不超过DataFrame的长度
...
def process_tsv(working_directory, bedGraph,species):
    ...
    # move bigwig folder to current directory and remove supercell_fragment folder
